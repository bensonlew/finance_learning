...
import time
'''
策略以DCE.i1801为交易标的，根据其一分钟(即60s频度）bar数据建立双均线模型，
周期为30，长周期为60，当短期均线由上向下穿越长期均线时做空，
...
def on_bar(context, bars):
    ...
    if slow_avg[-2] < fast_avg[-2] and slow_avg[-1] >= fast_avg[-1]:
        # 平多仓
        order_target_percent(symbol=context.symbol, percent=0, position_side=1,r_type=2)
        # 开空仓
        order_target_percent(symbol=context.symbol, percent=0.1, position_side=2,r_type=2)

    # 均线上穿，做多
    if fast_avg[-2] < slow_avg[-2] and fast_avg[-1] >= slow_avg[-1]:
        # 平空仓
        order_target_percent(symbol=context.symbol, percent=0, position_side=1,r_type=2,order_type=2)
        # 开多仓
