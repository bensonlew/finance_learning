...
class Addmoredata(PandasData):
    lines = ('K', 'close_rolling_480_std_mean', 'amount_normalize60_rolling_480_mean', 'close6_close96', 'close24_close480', 'close480_close2880', 'close24_close2880', 'amount_normalize20_rolling_96_mean')
    params = (
        ('K', 'K'), ('close_rolling_480_std_mean', 'close_rolling_480_std_mean'), 
        ...
        ('amount_normalize20_rolling_96_mean', 'amount_normalize20_rolling_96_mean')
    )


class MyStrategy(bt.Strategy):
    def __init__(self):
        self.kdj = bt.indicators.Stochastic(period=9*6, period_dfast=3*6, period_dslow=3*6)
    ...
    def next(self):
        ...
        if self.buy_period == "type1" and self.isbuyaa == False:
            pass
            if 1:
                ...
                if 1:
                    ...
                    amount_pre100 = self.data.amount_normalize20_rolling_96_mean.get(size=100)
                    if amount_pre100:
                        amount_pre100_max = max(amount_pre100)
                        amount_pre100_min = min(amount_pre100)
                        if (amount_pre100[-1] - amount_pre100_min)* 0 < amount_pre100_max - amount_pre100_min:
                            if self.data.close480_close2880[0] > 0 and self.data.close24_close480 < 0:
                                self.isbuyaa = True
                                self.order=self.buy(size=ss)

        if self.buy_period == "type3" and self.isbuyaa == False:
            if 1:
            #if self.data.close_rolling_480_std_mean[0] > 0.0 and self.data.close24_close2880 > 0  and (not self.isbuyaa) :
        ...
        if self.buy_period == "typen":
            if now_day != self.buy_day and self.isbuyaa:
                ...
                if (self.data.close[0] - self.zhisun)/self.zhisun > 0.06:
                # print('当前账户余额：', self.broker.get_cash())
                #self.sell()
        ...
        if self.data.close < self.zhisun * 0.99:
            if now_day != self.buy_day and self.isbuyaa:
