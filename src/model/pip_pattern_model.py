...
class PipPatternModel:
    ...
    def train_model(self):
        ...
        retain_ks = [str(k) for k in range(1, pip_miner.k_range) if k % 20 == 0]
    ...
    def set_train_test_data(self,):
        ...
        self.train_data_files = [self.fdata_dir + "/{}.qfq.kdj.parquet".format(code) for code in self.train_code_list] 
        self.test_data_files = [self.fdata_dir + "/{}.qfq.kdj.parquet".format(code) for code in self.test_code_list]
        self.train_arrs,  self.train_amounts,  self.train_signal_chooses, self.train_data_list = self.get_train_data()
