...
def get_5minutes_data(code, type="etf", from_date="2010-01-01", to_date="2030-01-01"):
    if type == "stock":
        df = QA_fetch_stock_min(code, from_date, to_date, format="pd", frequence="5min")
    else:
        df = QA_fetch_etf_min(code, from_date, to_date, format="pd")
    df["pre_datetime"] = df["datetime"].shift() 
    return df


def get_transaction_data(code, type="etf", from_date="2010-01-01", to_date="2030-01-01"):
    if type == "stock":
        df = QA_fetch_stock_transaction(code, from_date, to_date, format="pd")
    else:
        df = QA_fetch_etf_transaction(code, from_date, to_date, format="pd")
    # df["pre_datetime"] = df["datetime"].shift() # type: ignore
    # 统计df数据中按照 date分组的均值和标准差
    def get_mean_std(group):
        group = group[group["buyorsell"].isin([0,1])]
        group = group[group["vol"]>0]
        group["vol"] = np.log(group["vol"])
        return pd.Series({'mean': group["vol"].mean(), 'std': group["vol"].std(), "sum": group["vol"].sum()})
    
    print(df)

    df_grouped = df.groupby("date").apply(get_mean_std) # type: ignore
    df_grouped["mean_5"] = df_grouped["mean"].shift(1).rolling(window=5).mean()
    df_grouped["std_5"] = df_grouped["std"].shift(1).rolling(window=5).mean()


    df["diff_1"] =  df["price"].diff()/df["price"]
    df["diff_2"] = df["diff_1"].diff()
   
    df['diff_1_mean_5_after'] = df['diff_1'].shift(-5).rolling(window=5).mean()
    df['diff_1_mean_5_before'] = df['diff_1'].shift(1).rolling(window=5).mean()
    # df["diff_1_mean"] = df["diff_1"].rolling(5, center=True).mean()[:10]
    df["diff_2_mean"] = df["diff_2"].rolling(9, center=True).mean()

    return df, df_grouped


def etf_add_larget_ticket_feature(code="", type="etf"):
    """
    给etf数据添加大单交易特征
    :param df: DataFrame
    :param threshold: int
    ...
    """
    # 计算大单交易比例
    df = get_5minutes_data(code, type)
    transaction_df, df_grouped = get_transaction_data(code, type)
    large_stat_time_df = df.apply(large_stat_time, transaction_df=transaction_df, day_stat_df=df_grouped, axis=1)
    return large_stat_time_df 
    # 计算大单交易的价格分布
    # price_distribution = large_ticket_price_distribution(df, threshold)
    
    # 添加大单交易的价格分布特征
    # for i in range(price_distribution.shape[0]):
    #     df['large_ticket_price_distribution_{}'.format(i)] = price_distribution.iloc[i]
