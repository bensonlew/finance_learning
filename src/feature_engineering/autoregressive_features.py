...
def add_rolling_features(
    ...
    Args:
        ...
        n_shift (int, optional): Number of time steps to shift before computing rolling statistics.
            Typically used to avoid data leakage. Defaults to 1. 修改为0计算当前数据
        use_32_bit (bool, optional): Flag to use float32 or int32 to reduce memory. Defaults to False.
    ...
    if ts_id is None:
        ...
        rolling_df = pd.concat(
            [
                ...
                .shift(n_shift)
                .rolling(l)
                .agg({f"{column}_rolling_{l}_{agg}": agg_fun_dict[agg] for agg in agg_funcs})
                for l in rolls
            ],
            axis=1,
        )
...
def add_corr_feature(
    ...
    if ts_id is None:
        ...
        rolling_df.columns = [column + "_" + column2 + "_corr_roll" + str(l) for l in rolls ]
    else:
        assert (
            ts_id in df.columns
        ), "`ts_id` should be a valid column in the provided dataframe"
